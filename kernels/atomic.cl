...
inline void atomic_add_local(volatile __local float* source, const float add )
{
    union { unsigned int i; float f; } preVal;
    union { unsigned int i; float f; } newVal;
    do {
        preVal.f = *source;
        newVal.f = preVal.f + add;
    } while (atomic_cmpxchg( (volatile __local unsigned int*)source, preVal.i, newVal.i ) != preVal.i);
}
